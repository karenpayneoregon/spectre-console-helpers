...
public class Prompts
{
    ...
    public static string GetSSN(string title = "Enter your SSN")
    {
    ...
    public static int GetPin(string title = "Enter your Pin")
    {
        return AnsiConsole.Prompt(
            new TextPrompt<int>($"[{PromptColor}]{title}[/]:")
                .PromptStyle(PromptStyleColor)
    ...
    private static ValidationResult ValidatePin(int pin) => 
        SpecialValidation.IsValidPin(pin) ? ValidationResult.Success() : ValidationResult.Error();

    private static ValidationResult ValidateSocialSecurityNumber(string ssn)
    {
        if (SpecialValidation.IsValidSocialSecurityNumber(ssn))
        {
